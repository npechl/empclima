...
# for(i in unique(sam0$empo_3)) {
            ...
            labels = function(x) {
                ...
                return(
                    paste0(
                        ...
                        "\n",
                        "(", round(100 * x / nrow(abundance), digits = 2), "%)"
                    )
                )
            }
    ...
    ggsave(
        plot = gr, filename = paste0(
            workdir, 
            "/prevalence-curve-log10-", empo3_ontology, ".jpeg"
        ), width = 8, height = 8
    )
    ...
    colnames(prevalence_curve) = c("No. of Samples", "No. of ESVs")
    
    prevalence_curve = prevalence_curve[order(prevalence_curve$`No. of Samples`)]
    
    prevalence_curve$`Freq of Samples` = round(
        prevalence_curve$`No. of Samples` / length(sample.subset),
        digits = 6
    )
    
    prevalence_curve$`Freq. of ESVs` = round(
        prevalence_curve$`No. of ESVs` / nrow(abundance),
        digits = 6
    )
    
    
    # prevalence_curve$`cumsumFreq Samples` = cumsum(prevalence_curve$`Freq of Samples`)
    prevalence_curve$`cumsumFreq ESVs`    = cumsum(prevalence_curve$`Freq. of ESVs`)
    
    
    fwrite(
        prevalence_curve, paste0(workdir, "/prevalence.csv"),
