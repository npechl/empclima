...
tmp <- lapply(df[, 2:ncol(df)], function(x) { return(x / sum(x)) })
tmp <- setDT(tmp)

df  <- cbind(df[, 1], tmp)
df2 <- melt(
    df, id.vars = "TaxaIDabv", 
    variable.name = "Sample", value.name = "value",
    variable.factor = FALSE, value.factor = FALSE
)

df2 <- merge(df2, net_degree, by.x = "TaxaIDabv", by.y = "Taxa")

gr  <- ggplot(data = df2[which(value != 0)], aes(x = k, y = value)) +

    geom_point(
        shape = 21, size = 2, stroke = .1, 
        fill = "grey10", color = "grey95"
    ) +
...
gr <- ggplot(data = net_degree, aes(x = k, y = N)) + 
...
fwrite(
    plot_obj, paste0(workdir, "/sub-networks-plot.txt"),
    row.names = FALSE, quote = FALSE, sep = "\t"
)
