...
df = melt(
    df, id.vars = c("Taxa", "ClimateZone", "Group"),
    variable.name = "level", value.name = "label",
    variable.factor = FALSE, value.factor = FALSE
)

df$label = ifelse(
    str_length(df$label) == 0, "unassigned",
    paste0(
        str_to_lower(str_sub(df$level, 1, 1)), "__",
        df$label
    )
)
...
df2 = df[, by = .(ClimateZone, Group, level, label), .(N = .N)]
df2 = df2[, by = .(ClimateZone, Group, level), Freq := N / sum(N)]
...
df2$ann = ifelse(
    df2$Freq >= .25, df2$label, "Other"
)

cols = unique(df2[, c("level", "ann"), with = FALSE])
cols = cols[order(level), ]
cols$index = rowid(cols$level)

max(cols$index)

cols$col = pal_igv("default")(51)[cols$index]

cols[which(cols$ann == "unassigned"), ]$col = "grey"
cols[which(cols$ann == "Other"), ]$col = "white"
        
cols = unique(cols[, c("ann", "col"), with = FALSE])
    
my_col = cols$col
names(my_col) = cols$ann

df2$ann = factor(
    df2$ann,
    levels = c(
        cols[which(!(cols$ann %in% c("Other", "unassigned"))), ]$ann,
        "Other", "unassigned"
    )
)

gr = ggplot(data = df2, aes(x = level, y = Freq)) +
    
    geom_bar(
        aes(fill = ann), position = "fill", stat = "identity",
        color = "grey10", linewidth = .01
    ) +
...
ggsave(
    plot = gr, filename = "Clustering-taxonomy.jpeg",
